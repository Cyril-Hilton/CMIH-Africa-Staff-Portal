<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <h2 class="text-3xl font-display text-brand-white">Edit Project Budget</h2>
            </div>
            <a href="{{ route('portal.finance.budgets.show', $budget) }}" class="rounded-full border border-brand-white/20 px-4 py-2 text-xs uppercase tracking-[0.3em] text-brand-white/70 hover:bg-brand-white/10 transition-all">
                Back to Budget
            </a>
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto">
        <div class="glass-panel rounded-2xl p-6 border border-brand-white/10 bg-brand-white/5">
            <h3 class="text-lg font-semibold text-brand-white mb-6">Modify Budget Details</h3>

            <form action="{{ route('portal.finance.budgets.update', $budget) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $budget->title) }}" required placeholder="e.g. Q3 Brand Activation Budget" 
                    ...
                    <div>
                        ...
                        <select id="currency" name="currency" class="w-full rounded-xl border border-brand-white/10 bg-brand-black/80 text-brand-white px-4 py-2.5 focus:border-amber-500 focus:outline-none text-sm">
                            @php
                                $selectedCurrency = old('currency', $budget->currency);
                                if ($selectedCurrency === 'GHC') {
                                    $selectedCurrency = 'GH₵';
                                }
                            @endphp
                            @foreach (['GH₵' => 'GH₵ – Ghana Cedi', 'USD' => 'USD – US Dollar', 'EUR' => 'EUR – Euro', 'GBP' => 'GBP – British Pound', 'NGN' => 'NGN – Nigerian Naira', 'ZAR' => 'ZAR – South African Rand', 'XOF' => 'XOF – West African CFA franc', 'SLE' => 'SLE – Sierra Leonean Leone'] as $val => $lbl)
                                <option value="{{ $val }}" {{ $selectedCurrency === $val ? 'selected' : '' }}>
                                    {{ $lbl }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="task_id" name="task_id" class="w-full rounded-xl border border-brand-white/10 bg-brand-black/80 text-brand-white px-4 py-2.5 focus:border-amber-500 focus:outline-none text-sm">
                            ...
                            @foreach ($tasks as $task)
                                <option value="{{ $task->id }}" {{ old('task_id', $budget->task_id) == $task->id ? 'selected' : '' }}>
                                    {{ $task->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="notes" name="notes" value="{{ old('notes', $budget->notes) }}" placeholder="Brief summary of expense..." 
                            class="w-full rounded-xl border border-brand-white/10 bg-brand-black text-brand-white px-4 py-2.5 focus:border-amber-500 focus:outline-none text-sm" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="rounded-xl overflow-hidden border border-brand-white/10 bg-brand-black">
                        <textarea id="content" name="content" rows="12" class="wysiwyg-editor w-full bg-brand-black text-brand-white p-4 focus:outline-none">{{ old('content', $budget->content) }}</textarea>
                    </div>
                </div>
                ...
                <div class="glass-panel rounded-xl p-4 border border-brand-white/10 bg-brand-white/[0.02] space-y-3">
                    <div class="flex items-center justify-between border-b border-brand-white/5 pb-2">
                        <label class="block text-xs uppercase tracking-wider text-brand-ash font-bold">📥 Bulk Import and Append Items from Device (Excel / Doc / PPT / Images / PDF / Text)</label>
                ...
                <div class="glass-panel rounded-xl p-4 border border-brand-white/10 bg-brand-white/[0.02] space-y-3">
                    <h4 class="text-xs uppercase tracking-wider text-brand-ash font-bold border-b border-brand-white/5 pb-2">📊 Newly Imported Line Items Preview (Will append on save)</h4>
                    <div id="imported_items_preview_container">
                        <p class="text-xs text-brand-white/30 italic">No new line items loaded yet. Use the file import tool above to parse spreadsheets.</p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-brand-white/10">
                    <a href="{{ route('portal.finance.budgets.show', $budget) }}" class="px-5 py-2.5 rounded-xl text-xs uppercase tracking-wider text-brand-white/60 hover:text-brand-white bg-brand-white/5 hover:bg-brand-white/10 transition">
                        Cancel
                    </a>
    ...
    <script>
        ...
        function updateItemsPreview() {
            ...
            if (importedLineItems.length === 0) {
                container.innerHTML = `<p class="text-xs text-brand-white/30 italic text-center py-4">No new line items imported yet.</p>`;
                return;
            }
